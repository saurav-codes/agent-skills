...
| [ai-company-with-tmux](https://github.com/saurav-codes/ai-company-with-tmux) | Runs a hierarchical AI agent company over tmux agent windows: a tech lead hands off to a senior engineer that executes with workers and a strongest-model reviewer, communicating via handoff and inbox files | `npx skills add saurav-codes/ai-company-with-tmux -g` |
| [tmux-agents](https://github.com/saurav-codes/tmux-agents) | Coordinates parallel coding agents as tmux windows: spawn per-task windows, read what every agent is doing, send prompts, wait for output, clean up | `npx skills add saurav-codes/tmux-agents -g` |
